...
def patch_visualization_expert(patch: GeoPatch) -> None:
    ...
    if patch is not None:
        ...
        if patch.vector_data is not None:
            ...
            if 'boundary' in patch.vector_data:
            ...
            # add points
            if 'points' in patch.vector_data:
...
def threshold_expert(patch: GeoPatch, threshold: float, mode: str = 'greater') -> GeoPatch:
    '''
    ...
    data = patch.get_raster_data()['data']
    threshold_val = threshold * (np.nanmax(data) - np.nanmin(data))
    ...
    thresholded_patch.set_raster_data({
        'name': f"{patch.raster_data['name']} {'>' if mode == 'greater' else '<'} {threshold_val}",
        'data': thresholded_data,
        'type': RasterType.binary,
        'colormap': 'binary' # invert if needed
    })
    
    return thresholded_patch


def intersection_expert(patch1: GeoPatch, patch2: GeoPatch, mode='vector') -> GeoPatch:
    '''
    ...
    if mode == 'vector':
        data1 = patch1.get_vector_data() # shapely.geometry.Polygon
        data2 = patch2.get_vector_data()
        ...
        points1 = patch1.vector_data['points']
        points2 = patch2.vector_data['points']

        if points1 is None:
            points = points2 if points2 is not None else None
        else:
            points = points1 if points2 is None else points1 + points2
        if points is not None:
            points = [point for point in points if intersection_boundary.contains(point)]
        ...
        intersection_data = np.logical_and(data1, data2)
